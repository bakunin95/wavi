			...
			async.series([ 
				...
				function(cb) {
					async.map(nodes, function(node,cbmap2) {
						if(node.group == 1){
							g.addNode(node.name, {"label":'{&laquo;HTML»\\n'+node.name+'}',"fontsize":"12","shape":"record","style":"filled","fillcolor" : "lavenderblush"} );
						}
						if(node.group == 2){
						...
						if(node.group == 3){
							...
							g.addNode( node.name, {"label":'{&laquo;CSS»\\n'+node.name+'|'+rules.join('\\l')+'\\l|'+'\\l}',"fontsize":"12","shape":"record","style":"filled","fillcolor" : "palegreen"} );
